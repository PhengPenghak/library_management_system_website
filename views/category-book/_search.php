...
<div class="order-search">
    ...
    <div class="row">
        ...
        <div class="col-lg-4">
            <?= $form->field($model, 'globalSearch')->textInput(['placeholder' => 'ស្វែងរកប្រភេទសៀវភៅ ...............', 'class' => 'form-control form-control-lg'])->label('ស្វែងរក') ?>
        </div>
...
$script = <<<JS
    ...
    var is_filter = $("#categorybooksearch-from_date").val() != ''?true:false;

    if(!is_filter){
        var start = moment().startOf('week');
        var end = moment();
    }else{
        var start = moment($("#categorybooksearch-from_date").val());
        var end = moment($("#categorybooksearch-to_date").val());
    }

    function cb(start, end) {
        $('#order__date__range span').html(start.format('MMM D, YYYY') + ' - ' + end.format('MMM D, YYYY'));
        $("#categorybooksearch-from_date").val(start.format('YYYY-MM-D'));
        $("#categorybooksearch-to_date").val(end.format('YYYY-MM-D'));
    }
