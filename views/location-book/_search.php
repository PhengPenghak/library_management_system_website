...
<div class="order-search">
    ...
    <div class="row">
        ...
        <div class="col-lg-4">
            <?= $form->field($model, 'globalSearch')->textInput(['placeholder' => 'ស្វែងរកទីតាំងដាក់សៀវភៅ ...............', 'class' => 'form-control form-control-lg'])->label('ស្វែងរក') ?>
        ...
        <div class="col-lg-4 ml-auto">
            <div class="float-right">
                ...
                <a href="#" data-title="បញ្ចូលទីតាំងដាក់សៀវភៅថ្មី" value="<?= Url::to(['create']) ?>" class="btn btn-lg btn-primary modalButton"><i class="bi bi-plus-square mr-2"></i>ទីតាំងសៀវភៅ</span></a>
            </div>
        </div>
    </div>
...
$script = <<<JS
    ...
    var is_filter = $("#locationbooksearch-from_date").val() != ''?true:false;

    if(!is_filter){
        var start = moment().startOf('week');
        var end = moment();
    }else{
        var start = moment($("#locationbooksearch-from_date").val());
        var end = moment($("#locationbooksearch-to_date").val());
    }

    function cb(start, end) {
        $('#order__date__range span').html(start.format('MMM D, YYYY') + ' - ' + end.format('MMM D, YYYY'));
        $("#locationbooksearch-from_date").val(start.format('YYYY-MM-D'));
        $("#locationbooksearch-to_date").val(end.format('YYYY-MM-D'));
    }
